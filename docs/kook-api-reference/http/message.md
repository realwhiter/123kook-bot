...
| [/api/v3/message/pin](#置顶频道消息)         |   置顶频道消息          | 正常     |
| [/api/v3/message/unpin](#取消置顶频道消息)         | 取消置顶频道消息           | 正常     |
...
同时如果创建和更新消息是对之前5分钟内用户发送到当前频道的消息的第一次回复，也会在计算发送量时给于大幅折扣。
...
|reply_msg_id|string|否| POST | 当前消息回复的用户5分钟内发送到相同频道的消息的msg_id，如果是当前开发者的第一次回复，扣减当日发送量按n分之一条计算 |
...
| 地址                     | 请求方式 | 说明 |
| ------------------------ | -------- | ---- |
| `/api/v3/message/pin` | POST     |  需要 **管理消息** 权限。    |
...
| 地址                     | 请求方式 | 说明 |
| ------------------------ | -------- | ---- |
| `/api/v3/message/unpin` | POST     |  需要 **管理消息** 权限。    |
